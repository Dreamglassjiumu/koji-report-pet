...
当前版本：**Koji Report Pet v0.5.3 Koji 持续动态动画系统版**。

v0.5.3 在 v0.5.2 单一台词与文案组标签版之上，为 Koji 增加持续 CSS 动画系统：待机不再是静态图片，拖动、写日报、困倦、开心和催促等状态会呈现不同节奏。
...
- 新增 Koji 持续动态动画系统：13 个状态会通过状态 class + CSS keyframes 播放不同节奏的动作。
- `idle / 待机` 会持续轻微浮动、呼吸和小幅摇摆，让 Koji 在桌面上保持“活着”的感觉。
- `writing / 写日报` 增加快速小幅抖动，表现赶稿、疯狂写字的节奏。
- `sleep / 困倦` 增加慢速下沉和点头，像打瞌睡一样。
- `drag / 拖动` 增加被拎起来时的挣扎晃动；拖动过程中不会反复刷新台词，拖动结束会回到拖动前状态或待机。
- `happy / 开心` 增加弹跳庆祝；`angry / 催日报` 增加小幅前冲式催促抖动。
- 新增 Koji 动画强度设置，保存到本地设置并同步到独立桌宠窗口：
  - 关闭
  - 轻微
  - 标准
  - 活泼
- 本轮不需要新增图片素材，继续复用 `assets/koji/` 下现有 PNG 状态图；动画完全通过 CSS `transform` / `opacity` 实现。
- Koji 图片外层继续保持透明，不恢复黄色圆角底板、不添加头像壳或可见卡片背景。
- 未来可以在当前状态 class 基础上升级为多帧 WebP / GIF，或进一步拆成分层骨骼动画。
...
pet.css                 # 独立桌宠窗口透明 UI、拖动区域、快速记录面板、状态动画和动画强度变量
pet.js                  # 独立桌宠窗口状态配置、动画 class 同步、快速记录、右键菜单触发、双击打开面板和 IPC 移动
README.md               # 项目说明
